...
- ``mimic_distribution_lookup(itemid, cohort=None, icd10_prefixes=None, icd9_prefixes=None)`` — empirical MIMIC distribution (n, mean, p50, p95, units) for a chartevents/labevents itemid. Use when you need the cohort-typical distribution to settle "shifted but plausible vs polluted." If you don't know the itemid, **first call ``mimic_itemid_search(query='<analyte>')``** to look up the itemid before this tool.
  ...
  **Worked chain — procalcitonin in septic shock.** Question: "Is mean procalcitonin 12.4 ng/mL plausible in our septic shock cohort?" Step 1: ``mimic_itemid_search(query="procalcitonin")``. If empty results → procalcitonin isn't in MIMIC; pivot to ``pubmed_search`` (published procalcitonin-mortality strata). If non-empty → take the resolved itemid and call ``mimic_distribution_lookup(itemid=<resolved>, cohort="septic shock")``.
